...
{
    public class FindMySentMessagesQuery : SearchQuery<Message>
    {
        public Guid UserId { get; set; }

        public FindMySentMessagesQuery()
        {
            SortColumn = "Date";
        ...
        public override ISpecification<Message> ToSpec()
        {
            ...
            if (UserId == Guid.Empty) throw new ArgumentNullException(nameof(UserId), "UserId must be specified.");

            ISpecification<Message> spec = new Specification<Message>(o => o.Sender.Id == UserId);

            if (SearchCriteria.IsNotNullOrWhiteSpace())
            {
                ISpecification<Message> criteriaSpec = new Specification<Message>(o => o.Subject.ToUpper().Contains(SearchCriteria.ToUpper()));
                ...
                //criteriaSpec = criteriaSpec.Or(o => ("{0}{1}{2}".FormatWith(o.Sender.FirstName, o.Sender.LastName, o.Sender.Email).ToUpper().Contains(SearchCriteria.ToUpper())));
